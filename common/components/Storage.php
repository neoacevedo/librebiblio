...
class Storage extends \yii\base\BaseObject {
     ...
     * @param string|object $file El nombre del archivo real.
     * Este nombre se obtiene del archivo subido:
     * <code>"images/" . $model->imageFile->baseName . '.' . $model->imageFile->extension</code>
     * <br />
     * Si $file es de tipo \yii\web\UploadedFile se está haciendo uso del servicio local del almacenamiento.
     */
    ...
    public function getUrl($file) {
        switch ($this->service) {
            case self::AWS_S3:
                ...
                break;
            case self::AZURE_BLOB_STORAGE:
                return $this->clientService->getBlobUrl($this->bucket, $file);
                break;
            case self::GOOGLE_CLOUD_STORAGE:
    ...
    private function uploadToAzure($file) {
        try {
            ...
            $this->clientService->createBlockBlob($this->bucket, $file, $blob_content);
    ...
    private function uploadToGoogle($file) {
        try {
            $content = file_get_contents($file);
            $this->clientService->bucket($this->bucket, true)->upload($content, ['name' => $file, 'predefinedAcl' => 'publicRead']);
